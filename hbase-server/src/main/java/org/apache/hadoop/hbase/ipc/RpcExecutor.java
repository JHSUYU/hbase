...
public abstract class RpcExecutor {
    ...
    protected class Handler extends Thread {
        ...
        private boolean isRetryField(Map.Entry<Descriptors.FieldDescriptor, Object> entry) {
            Descriptors.FieldDescriptor fieldDescriptor = entry.getKey();
            Object value = entry.getValue();

            return fieldDescriptor.getName().equals("is_retry") && Boolean.TRUE.equals(value);
        }

        public void run$Checker(CallRunner callRunner) {
            if (this.isDryRun(callRunner)) {
                this.run$instrumentation(callRunner);
            } else {
                this.run(callRunner);
            }
        }
        ...
        private void run$instrumentation(CallRunner cr) {
            MonitoredRPCHandler status = RpcServer.getStatus();
            cr.setStatus(status);
            try {
                this.activeHandlerCount.incrementAndGet();
                cr.run$instrumentation();
            } catch (Throwable e) {
                if (e instanceof Error) {
                    int failedCount = failedHandlerCount.incrementAndGet();
                    if (this.handlerFailureThreshhold >= 0
