...
public abstract class FSUtils {
    ...
    private static void startDryRun(final FileSystem fs, final Path rootdir, final String version,
                                    final int wait, final int retries) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    if (wait > 0) {
                        LOG.warn("Dry run: Unable to create version file at " + rootdir.toString() +
                                ", retrying: ");
                        // Use Undo Log to remember the operation
                        LOG.info("Dry run: Simulating delete of version file");
                        try {
                            Thread.sleep(wait);
                    ...
                    setVersion$instrumentation(fs, rootdir, version, wait, retries);
                } catch (IOException e) {
                    LOG.error("Dry run: Error occurred during simulated setVersion", e);
                }
            }
        }).start();
    }
    ...
    public static void setVersion(FileSystem fs, Path rootdir, String version,
        ...
        while (true) {
            try {
                ...
                // TODO need support from Clone Library, or use Undo Log to record the operation in FileSystem during dry run
                FileSystem shadowFS = fs;
                startDryRun(shadowFS, rootdir, version, wait, retries);
                if (wait > 0) {
...
//        }
    ...
    public static void setVersion$instrumentation(FileSystem fs, Path rootdir, String version,
                                                  int wait, int retries) throws IOException {
        Path versionFile = new Path(rootdir, HConstants.VERSION_FILE_NAME);
        Path tempVersionFile = new Path(rootdir, HConstants.HBASE_TEMP_DIRECTORY + Path.SEPARATOR +
                HConstants.VERSION_FILE_NAME);
