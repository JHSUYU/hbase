...
public class RpcRetryingCaller<T> {
    ...
    public T callWithRetries(RetryingCallable<T> callable, int callTimeout)
        ...
        for (int tries = 0; ; tries++) {
            long expectedSleep;
            try {
                //Currently we just add dry-run checker for ScanRequest
                if (callable instanceof ScannerCallableWithReplicas.RetryingRPC) {
                    ScannerCallableWithReplicas.RetryingRPC retryingRPC = (ScannerCallableWithReplicas.RetryingRPC) callable;
                    if(dryRunMode){
                        //once we retry the request, we will do a dry run, so in RequestConverter.buildScanRequest we will make the is_dry_run field to be true,
                        //letting the server-side know we have a dry-run request first.
                        retryingRPC.setDryRun();
                        dryRunMode = false;
                        //this retry will be a dry-run request, if the dry-run fails, we will catch a DryRunCheckerException and the Scan request will fail without retrying, otherwise we just revert the tries
                        //and let the real retry go for the next time
                        tries--;
                    }else{
                        dryRunMode = true;
                    }
